...
class AddForm extends Component {
    ...
    handleAddItem(vals){
        ...
        this.props.addTodo(vals).then(()=>{
            //this is how to redirect somewhere
            this.props.history.push('/');
        });
    }

    renderInput({input, label, type, meta:{touched, error}}/*pulls things out of the props that is automatically passed in I think*/){
        ...
        return (
            <div className={`form-group ${hasError ?'has-danger':''}`}>
                ...
                <input {...input} className={`form-control ${hasError ? 'form-control-danger' : ''}`} type={type?type:'text' /*so that type defaults to text if it's not set*/}/>
